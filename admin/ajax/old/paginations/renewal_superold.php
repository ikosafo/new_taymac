...
## Fetch records
    ...
    $sel = mysqli_query($con,"SELECT COUNT(*) as allcount FROM renewal r
                              JOIN provisional p ON r.applicant_id = p.applicant_id
                              WHERE r.cpdyear != '' AND r.cpdyear = '$year' AND r.cpd_admincheck_status = 'Approved'");
    $records = mysqli_fetch_assoc($sel);
    $totalRecords = $records['allcount'];
...
## Fetch records
    $empQuery = "SELECT r.cpd_usercheck_status,r.cpd_admincheck_status,r.period_registered,p.*
                     FROM renewal r
                     JOIN provisional p ON r.applicant_id = p.applicant_id
                     WHERE r.cpdyear != '' AND r.cpdyear = '$year' AND r.cpd_admincheck_status = 'Approved' AND 1 ".$searchQuery."
                     ORDER BY r.period_registered DESC limit ".$row.",".$rowperpage;
    $empRecords = mysqli_query($con, $empQuery);
    $data = array();
    ...
    $sel = mysqli_query($con,"SELECT COUNT(*) as allcount FROM renewal r
                              JOIN provisional p ON r.applicant_id = p.applicant_id
                              WHERE r.cpdyear != '' AND r.cpdyear = '$year' AND r.cpd_admincheck_status = 'Rejected'");
    $records = mysqli_fetch_assoc($sel);
    $totalRecords = $records['allcount'];
...
## Fetch records
    $empQuery = "SELECT r.cpd_usercheck_status,r.cpd_admincheck_status,r.period_registered,p.*
                     FROM renewal r
                     JOIN provisional p ON r.applicant_id = p.applicant_id
                     WHERE r.cpdyear != '' AND r.cpdyear = '$year' AND r.cpd_admincheck_status = 'Rejected' AND 1 ".$searchQuery."
                     ORDER BY r.period_registered DESC limit ".$row.",".$rowperpage;
    $empRecords = mysqli_query($con, $empQuery);
    $data = array();
    ...
    $sel = mysqli_query($con,"SELECT COUNT(*) as allcount FROM renewal r
                              ...
                              AND (r.cpd_admincheck_status = '' OR r.cpd_admincheck_status IS NULL)");
    $records = mysqli_fetch_assoc($sel);
    $totalRecords = $records['allcount'];
