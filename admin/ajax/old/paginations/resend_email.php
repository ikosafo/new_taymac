...
where (permanent_admincheck_status = 'Approved' OR provisional_admincheck_status = 'Approved' OR renewal = '1')");
...
WHERE (permanent_admincheck_status = 'Approved' OR provisional_admincheck_status = 'Approved' OR renewal = '1') AND 1 ".$searchQuery." order by
                               resend_mail limit ".$row.",".$rowperpage;
...
while ($row = mysqli_fetch_assoc($empRecords)) {
    $data[] = array(
        ...
        "resend_mail"=>resendmailstatus($row['resend_mail']),
        "res_region"=>$row['res_region'],
        /*"registration_mode"=>$row['registration_mode'],*/
        "applicant_id"=>resend_email($row['applicant_id'])
    );
}
