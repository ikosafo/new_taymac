...
where exam_index_number != ''");
$records = mysqli_fetch_assoc($sel);
$totalRecords = $records['allcount'];

## Total number of record with filtering
$sel = mysqli_query($con,"select count(*) as allcount from provisional
WHERE exam_index_number != '' AND 1 ".$searchQuery);
$records = mysqli_fetch_assoc($sel);
$totalRecordwithFilter = $records['allcount'];

## Fetch records
$empQuery = "select * from provisional WHERE exam_index_number != '' AND 1 ".$searchQuery." order by
                               surname,first_name limit ".$row.",".$rowperpage;
