...
## Total number of records without filtering
        $sel = mysqli_query($con,"select count(*) as allcount from registration_upgrades where 
                        (registration_type = 'provisional_upgrade' OR registration_type = 'PROVISIONAL(UPGRADE)') AND SUBSTRING(date_requested,1,4) = '$year'");
        $records = mysqli_fetch_assoc($sel);
        $totalRecords = $records['allcount'];
...
## Fetch records
    ...
    if ($year == "All") {
        ...
        $sel = mysqli_query($con,"select count(*) as allcount from registration_upgrades 
                where (registration_type = 'provisional_upgrade' OR registration_type = 'PROVISIONAL(UPGRADE)') AND admincheck_status = 'Approved'");
        $records = mysqli_fetch_assoc($sel);
        $totalRecords = $records['allcount'];
...
## Fetch records
    ...
    if ($year == "All") {
        ## Total number of records without filtering
        $sel = mysqli_query($con,"select count(*) as allcount from registration_upgrades where 
                (registration_type = 'provisional_upgrade' OR registration_type = 'PROVISIONAL(UPGRADE)') AND admincheck_status = 'Rejected'");
        $records = mysqli_fetch_assoc($sel);
        $totalRecords = $records['allcount'];
...
## Total number of records without filtering
        $sel = mysqli_query($con,"select count(*) as allcount from registration_upgrades where 
                (registration_type = 'provisional_upgrade' OR registration_type = 'PROVISIONAL(UPGRADE)') AND
...
while ($row = mysqli_fetch_assoc($empRecords)) {
    $data[] = array(
        ...
        "upgrade_id"=>superupapprovalpro($row['id'])
    );
}
