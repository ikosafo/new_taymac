...
## Total number of record with filtering
    $sel = mysqli_query($con,"select count(*) as allcount from provisional WHERE permanent_registration = '1' AND 1 ".$searchQuery);
    $records = mysqli_fetch_assoc($sel);
    $totalRecordwithFilter = $records['allcount'];

## Fetch records
    $empQuery = "select * from provisional WHERE permanent_registration = '1' AND 1 ".$searchQuery." order by surname, first_name,other_name, permanent_period DESC limit ".$row.",".$rowperpage;
...
while ($row = mysqli_fetch_assoc($empRecords)) {
    $data[] = array(
        ...
        "payment"=>getpayment($row['permanent_payment']),
        "mis_status"=>$row['permanent_usercheck_status'],
        "admin_status"=>$row['permanent_admincheck_status'],
        "pin"=>$row['provisional_pin'],
        "action"=>delperm($row['provisionalid'])
    );
}
