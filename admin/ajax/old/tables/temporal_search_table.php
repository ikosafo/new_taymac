    ...
    <div class="kt-section">
        <div class="kt-section__content responsive">
            ...
            if ($search_year == "All" && $search_region == "Any" && $search_approval == "Any" && $search_profession == "Any") {
                $query = $mysqli->query("select * from provisional where temporal_registration = '1' ORDER BY professionid,temporal_period DESC");
                $count = mysqli_num_rows($query);
            }

            //ALL THREE
            else if ($search_region == "Any" && $search_approval == "Any" && $search_profession == "Any") {
                $query = $mysqli->query("select * from provisional where SUBSTRING(temporal_period,1,4) = '$search_year'
ORDER BY professionid,temporal_period DESC");
                $count = mysqli_num_rows($query);
            }
            ...
            //Approval and Profession
                ...
                $query = $mysqli->query("select * from provisional where SUBSTRING(temporal_period,1,4) = '$search_year'
AND res_region = '$search_region' ORDER BY professionid,temporal_period DESC");
                $count = mysqli_num_rows($query);
            } //Region and Profession
            else if ($search_region == "Any" && $search_profession == "Any") {
                if ($search_approval == "Pending") {
                    $query = $mysqli->query("select * from provisional where SUBSTRING(temporal_period,1,4) = '$search_year'
AND temporal_admincheck_status IS NULL ORDER BY professionid,temporal_period DESC");
                    $count = mysqli_num_rows($query);
                } else {
                    if ($search_year == "All") {
                        $query = $mysqli->query("select * from provisional where temporal_admincheck_status = '$search_approval'
ORDER BY professionid,temporal_period DESC");
                        $count = mysqli_num_rows($query);
                    }
                    else {
                        $query = $mysqli->query("select * from provisional where SUBSTRING(temporal_period,1,4) = '$search_year'
AND temporal_admincheck_status = '$search_approval' ORDER BY professionid,temporal_period DESC");
                        $count = mysqli_num_rows($query);
                    }
...
SUBSTRING(temporal_period,1,4) = '$search_year' AND profession = '$search_profession'
ORDER BY professionid,temporal_period DESC");
                $count = mysqli_num_rows($query);
            } //ONE's
                ...
                if ($search_approval == "Pending") {
                    $query = $mysqli->query("select * from provisional where
SUBSTRING(temporal_period,1,4) = '$search_year'
AND profession = '$search_profession' AND temporal_admincheck_status IS NULL
ORDER BY professionid,temporal_period DESC");
                    $count = mysqli_num_rows($query);
                }
                else {
                    $query = $mysqli->query("select * from provisional where
SUBSTRING(temporal_period,1,4) = '$search_year'
AND profession = '$search_profession' AND temporal_admincheck_status = '$search_approval'
ORDER BY professionid,temporal_period DESC");
                    $count = mysqli_num_rows($query);
                }
...
ORDER BY professionid,temporal_period DESC");
                ...
                if ($search_approval == "Pending") {
                    $query = $mysqli->query("select * from provisional where SUBSTRING(temporal_period,1,4) = '$search_year'
AND temporal_admincheck_status IS NULL AND res_region = '$search_region'
ORDER BY professionid,temporal_period DESC");
                    $count = mysqli_num_rows($query);
                } else {
                    $query = $mysqli->query("select * from provisional where SUBSTRING(temporal_period,1,4) = '$search_year'
AND temporal_admincheck_status = '$search_approval' AND res_region = '$search_region'
ORDER BY professionid,temporal_period DESC");
                    $count = mysqli_num_rows($query);
                }
                ...
                if ($search_approval == "Pending") {
                    $query = $mysqli->query("select * from provisional where temporal_admincheck_status IS NULL
 AND res_region = '$search_region' AND profession = '$search_profession'
 AND SUBSTRING(temporal_period,1,4) = '$search_year'
ORDER BY professionid,temporal_period DESC");
                    $count = mysqli_num_rows($query);
                } else {
                    $query = $mysqli->query("select * from provisional where profession = '$search_profession'
 AND res_region = '$search_region' AND temporal_admincheck_status = '$search_approval' AND  SUBSTRING(temporal_period,1,4) = '$search_year'
ORDER BY professionid,temporal_period DESC");
                    $count = mysqli_num_rows($query);
                }
            }
            ...
            while ($result = $query->fetch_assoc()) {
                ...
                <tr>
                    ...
                    <td>
                        <?php $period = $result['temporal_period'];
                        echo time_elapsed_string($period); ?>
                    </td>
                    <td>
                        <?php $paid = $result['temporal_payment'];
                        if ($paid == "1") {
                            echo "Paid";
                        } else {
                            echo "Not Paid";
                    ...
                    <td>
                        <?php echo $approve = $result['temporal_admincheck_status'];
                        if ($approve == "") {
                            echo "Pending";
                        }
                        ?>
