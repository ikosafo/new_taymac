...
if ($admin_type == 'Super Admin') {
        ...
        if ($renewal_status == "Approved") {
            $query = $mysqli->query("SELECT * FROM renewal r
                                    ...
                                    WHERE r.cpdyear != '' AND r.cpdyear = '$year_search' 
            ...
            $query = $mysqli->query("SELECT * FROM renewal r
                                    ...
                                    AND (r.cpd_admincheck_status != 'Rejected' OR 
                                    r.cpd_admincheck_status != 'Approved')");
        }


}
else if ($admin_type == 'MIS Admin') {

    
        if ($renewal_status == "Approved") {
            $query = $mysqli->query("SELECT * FROM renewal r
                                    JOIN provisional p ON r.applicant_id = p.applicant_id
                                    WHERE r.cpdyear != '' AND r.cpdyear = '$year_search' AND r.cpd_usercheck_status = 'Approved'");
        }
        else if ($renewal_status == "Rejected") {
            $query = $mysqli->query("SELECT * FROM renewal r
                                    JOIN provisional p ON r.applicant_id = p.applicant_id
                                    WHERE r.cpdyear != '' AND r.cpdyear = '$year_search' AND r.cpd_usercheck_status = 'Rejected'");
        } else if ($renewal_status == "Not Paid") {
            $query = $mysqli->query("SELECT * FROM renewal r
                                    JOIN provisional p ON r.applicant_id = p.applicant_id
                                    WHERE r.cpdyear != '' AND r.cpdyear = '$year_search' AND r.payment = '0'");
