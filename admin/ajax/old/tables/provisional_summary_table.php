...
<div id="print_this">
    ...
    <div class="row">
        <div class="col-md-3">
            PROVISIONAL REGISTRATIONS FOR <b><?php echo $search_year ?></b>
        </div>
        <div class="col-md-3">
    ...
    <div class="kt-section">
        ...
        <div class="kt-section__content responsive">
            ...
            if ($search_year == "All" && $search_region == "Any" && $search_approval == "Any" && $search_profession == "Any") {
                $query = $mysqli->query("select * from provisional where provisional_registration = '1' ORDER BY professionid,provisional_period DESC");
                $count = mysqli_num_rows($query);
            }

            //ALL THREE
            else if ($search_region == "Any" && $search_approval == "Any" && $search_profession == "Any") {
                $query = $mysqli->query("select * from provisional where SUBSTRING(provisional_period,1,4) = '$search_year'
ORDER BY professionid,provisional_period DESC");
                $count = mysqli_num_rows($query);
            }
            ...
            //Approval and Profession
                ...
                $query = $mysqli->query("select * from provisional where SUBSTRING(provisional_period,1,4) = '$search_year'
AND res_region = '$search_region' ORDER BY professionid,provisional_period DESC");
                $count = mysqli_num_rows($query);
            } //Region and Profession
            else if ($search_region == "Any" && $search_profession == "Any") {
                if ($search_approval == "Pending") {
                    $query = $mysqli->query("select * from provisional where SUBSTRING(provisional_period,1,4) = '$search_year'
AND provisional_admincheck_status IS NULL ORDER BY professionid,provisional_period DESC");
                    $count = mysqli_num_rows($query);
                } else {
                    if ($search_year == "All") {
                        $query = $mysqli->query("select * from provisional where provisional_admincheck_status = '$search_approval'
ORDER BY professionid,provisional_period DESC");
                        $count = mysqli_num_rows($query);
                    }
                    else {
                        $query = $mysqli->query("select * from provisional where SUBSTRING(provisional_period,1,4) = '$search_year'
AND provisional_admincheck_status = '$search_approval' ORDER BY professionid,provisional_period DESC");
                        $count = mysqli_num_rows($query);
                    }
...
SUBSTRING(provisional_period,1,4) = '$search_year' AND profession = '$search_profession'
ORDER BY professionid,provisional_period DESC");
                $count = mysqli_num_rows($query);
            } //ONE's
                ...
                if ($search_approval == "Pending") {
                    $query = $mysqli->query("select * from provisional where
SUBSTRING(provisional_period,1,4) = '$search_year'
AND profession = '$search_profession' AND provisional_admincheck_status IS NULL
ORDER BY professionid,provisional_period DESC");
                    $count = mysqli_num_rows($query);
                }
                else {
                    $query = $mysqli->query("select * from provisional where
SUBSTRING(provisional_period,1,4) = '$search_year'
AND profession = '$search_profession' AND provisional_admincheck_status = '$search_approval'
ORDER BY professionid,provisional_period DESC");
                    $count = mysqli_num_rows($query);
                }
...
ORDER BY professionid,provisional_period DESC");
                ...
                if ($search_approval == "Pending") {
                    $query = $mysqli->query("select * from provisional where SUBSTRING(provisional_period,1,4) = '$search_year'
AND provisional_admincheck_status IS NULL AND res_region = '$search_region'
ORDER BY professionid,provisional_period DESC");
                    $count = mysqli_num_rows($query);
                } else {
                    $query = $mysqli->query("select * from provisional where SUBSTRING(provisional_period,1,4) = '$search_year'
AND provisional_admincheck_status = '$search_approval' AND res_region = '$search_region'
ORDER BY professionid,provisional_period DESC");
                    $count = mysqli_num_rows($query);
                }
                ...
                if ($search_approval == "Pending") {
                    $query = $mysqli->query("select * from provisional where provisional_admincheck_status IS NULL
 AND res_region = '$search_region' AND profession = '$search_profession'
 AND SUBSTRING(provisional_period,1,4) = '$search_year'
ORDER BY professionid,provisional_period DESC");
                    $count = mysqli_num_rows($query);
                } else {
                    $query = $mysqli->query("select * from provisional where profession = '$search_profession'
 AND res_region = '$search_region' AND provisional_admincheck_status = '$search_approval' AND  SUBSTRING(provisional_period,1,4) = '$search_year'
ORDER BY professionid,provisional_period DESC");
                    $count = mysqli_num_rows($query);
                }
            }
